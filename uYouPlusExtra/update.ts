...
// Steps:
// 1. Get all releases from https://api.github.com/repos/arichorn/uYouPlusExtra/releases
...
logger.info(Bold('Step 1: ') + DarkGray(`Get all releases from ${Underline('https://api.github.com/repos/arichorn/uYouPlusExtra/releases')}`))
axios.get('https://api.github.com/repos/arichorn/uYouPlusExtra/releases')
  .then(async (releases) => {
    ...
    const dest = fs.createWriteStream('./uYouPlusExtra.ipa')
    // file.data.pipe(dest)
    ...
    const zip = new AdmZip('./Out/uYouPlusExtra.ipa')
    ...
    app.downloadURL = `https://github.com/DeltAndy123/AltStore-Sources/releases/download/uyouplusextra-${release.tag_name.match(/v(.*?)-/)[1]}/uYouPlusExtra.ipa`
    ...
    app.versions.unshift({
      ...
      downloadURL: `https://github.com/DeltAndy123/AltStore-Sources/releases/download/uyouplusextra-${release.tag_name.match(/v(.*?)-/)[1]}/uYouPlusExtra.ipa`,
