# Steps:
# 1. Get the latest release from https://api.github.com/repos/arichorn/uYouPlusExtra/releases/latest
...
release=$(curl -s https://api.github.com/repos/arichorn/uYouPlusExtra/releases/latest)
echo "Done."

# Step 2
echo "Downloading the release file..."
url=$(echo $release | jq -r '.assets[0].browser_download_url')
curl -s -L -o uYouPlusExtra.ipa $url
echo "Done."

# Step 3
mkdir Out/
echo "Removing app extensions in the file..."
./azule -i uYouPlusExtra.ipa -o Out/ -e
echo "Done."

# Step 4
echo "Reading the app Info.plist..."
plist=$(unzip -p ./Out/uYouPlusExtra.ipa Payload/YouTube.app/Info.plist)
echo "Done."
...
json=$(echo $json | jq ".apps[0].downloadURL = \"https://github.com/DeltAndy123/AltStore-Sources/releases/download/uyouplusextra-$(echo $release | jq -r '.tag_name' | grep -oP 'v\K(.*?)(?=-)')/uYouPlus.ipa\"")
json=$(echo $json | jq ".apps[0].size = $(echo $release | jq -r '.assets[0].size')")

echo "Done."

echo "Writing the source json..."
echo $json > uyouplusextra.json
echo "Done."